...
def test_months_are_whole_weeks() -> None:
...
def test_month_starting_on_monday_has_no_blanks() -> None:
    block = month_block(*JUNE)
    assert block.rows[0][0].date == date(2026, 6, 1)
    assert len(block.rows) == 5


def test_month_starting_late_is_padded() -> None:
...
def test_no_day_appears_in_two_blocks() -> None:
    """A day borrowed by a neighbouring block makes a selection uncountable."""
    seen: list[date] = []
...
def test_leap_february_gains_a_day() -> None:
    block = month_block(2024, 2)
    assert sum(1 for row in block.rows for cell in row if cell.date is not None) == 29


def test_first_weekday_rotates_the_grid() -> None:
    """The columns move, and the headings with them."""
...
def test_spans_cover_whole_months() -> None:
...
def test_span_inside_one_month_is_one_block() -> None:
    blocks = stitch(date(2026, 6, 8), date(2026, 6, 14))
    assert len(blocks) == 1


def test_span_across_a_year_end() -> None:
...
def test_block_knows_its_own_bounds() -> None:
    block = month_block(*AUGUST)
...
def test_new_selection_is_one_day() -> None:
...
def test_extending_keeps_the_anchor() -> None:
    """The anchor is what lets a selection be pulled back to nothing."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Same month, a year apart: the year has to appear on both sides, or
        # this reads as a fortnight running backwards. Nothing clamps `extend`.
        (364, "Mon 10 Aug 2026 – Mon 9 Aug 2027"),
    ],
)
def test_selection_names_itself(extend: int, expected: str) -> None:
    """The repeated month is dropped, because a panel is thirty columns wide."""
