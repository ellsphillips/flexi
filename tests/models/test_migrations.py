...
# ---- 0011: the day tracking began ----

BEFORE_TRACKING = "0010"
...
def configured(db: Path) -> None:
    """A settings row as 0010 knew it, written without the ORM.

    Through raw SQL because the model has `tracking_since` and 0011 is what
    ...
    with engine.connect() as connection:
        connection.execute(
            sa.text(
                "INSERT INTO settings (id, leave_year_start, working_days,"
                " bank_holiday_division, auto_close_time, contracted_minutes,"
                " day_window_start, day_window_end)"
                " VALUES (1, '04-06', '0,1,2,3,4', 'england-and-wales',"
