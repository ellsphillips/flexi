...
async def open_leave(pilot) -> None:
...
async def test_f2_opens_the_leave_year(app_factory) -> None:
...
async def test_escape_leaves(app_factory) -> None:
...
async def test_the_whole_year_is_laid_out(app_factory) -> None:
...
async def test_the_cursor_moves(app_factory, key: str, days: int) -> None:
    app = app_factory()
    async with app.run_test(size=WIDE) as pilot:
        await open_leave(pilot)
...
async def test_shift_extends_and_escape_collapses(app_factory) -> None:
...
async def test_a_month_step_clamps_to_a_shorter_month(app_factory) -> None:
...
async def test_one_key_books_a_day(app_factory) -> None:
...
async def test_one_key_books_a_range(app_factory) -> None:
...
async def test_space_cycles_the_portion_before_booking(app_factory) -> None:
...
async def test_other_absence_goes_through_the_modal(app_factory) -> None:
...
async def test_the_wallet_moves_with_the_booking(app_factory) -> None:
...
async def test_removing_a_day_is_immediate(app_factory) -> None:
...
async def test_removing_a_lot_asks_first(app_factory) -> None:
...
async def test_the_seed_is_drawn(app_factory) -> None:
...
async def test_every_panel_is_jumpable(app_factory) -> None:
    app = app_factory()
    async with app.run_test(size=WIDE) as pilot:
        await open_leave(pilot)
...
async def test_the_rail_gives_way_when_there_is_no_room(app_factory) -> None:
...
async def test_the_grid_never_outgrows_its_panel(app_factory) -> None:
